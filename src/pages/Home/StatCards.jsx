...
const StatCards = () => {
  ...
  const { data } = useQuery({
    queryKey: ["stats"],
    queryFn: async () => {
      const res = await axiosPublic.get(`/stat-cards`);
      return res.data;
    },
  });

  return (
    <section className="w-[90%] min-[1920px]:max-w-[108rem] mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 text-center pt-10 sm:pt-14 md:pt-18 xl:pt-20 2xl:pt-24">
      <motion.div
      ...
      >
        ...
        <p className="text-sm sm:text-base xl:text-xl 2xl:text-2xl">
          Active Camps
        </p>
      </motion.div>
      <motion.div
      ...
      >
        ...
        <h3 className="text-2xl sm:text-3xl xl:text-4xl 2xl:text-5xl font-bold">
          {countStart ? (
            <CountUp
              start={0}
              end={data?.totalParticipantCount}
              duration={3}
              separator=","
            />
          ) : (
