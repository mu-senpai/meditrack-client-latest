...
const UserAnalytics = () => {
  ...
  useEffect(() => {
    if (user?.email) {
      axiosSecure
        .get(`/registered-camps/${user.email}`)
  ...
  const feesData = campData.map((camp) => ({
    name: camp.campName,
  ...
  const paymentData = [
    {
      name: "Paid",
      value: campData.filter((c) => c.paymentStatus === "Paid").length,
    },
    {
      name: "Unpaid",
      value: campData.filter((c) => c.paymentStatus === "Unpaid").length,
    },
  ];

  const monthlyRegistrations = campData.reduce((acc, camp) => {
    const month = new Date(camp.registrationTime).toLocaleString("default", {
      month: "short",
    });
  ...
  return (
    <div className="w-full p-6 min-h-[calc(100vh-130px)] py-8 sm:py-12 md:py-14 xl:py-16">
      <motion.h2
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        transition={{ duration: 0.5 }}
        className="text-2xl sm:text-3xl xl:text-4xl font-bold text-accent text-center mb-6 sm:mb-8 lg:mb-10 xl:mb-12"
      >
        Analytics Dashboard
      </motion.h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
        ...
        >
          ...
          <ResponsiveContainer width="100%" height={250}>
            <BarChart
              data={[{ name: "Registered", count: totalRegisteredCamps }]}
            >
              <XAxis dataKey="name" />
